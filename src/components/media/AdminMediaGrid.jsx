...
import { FaEdit, FaTrashAlt, FaMapMarkerAlt, FaCity, FaRulerCombined, FaCheck } from "react-icons/fa";
...
const AdminMediaGrid = ({ filters, selectedIds, toggleSelection }) => {
  ...
  const handleDeleteClick = useCallback(async (e, media) => {
    e.preventDefault();
    e.stopPropagation();
    const confirmed = window.confirm(`Are you sure you want to delete "${media.mediaCode}"? This cannot be undone.`);
    if (!confirmed) return;
    try { await deleteMedia(media.id); setMediaList((prev) => prev.filter((m) => m.id !== media.id)); setTotalItems((prev) => prev - 1); }
    catch (error) { console.error("Failed to delete media:", error); alert("Failed to delete media. Please try again."); }
  }, []);

  const handleCardClick = useCallback((media) => { if (toggleSelection && media?.id != null) toggleSelection(media.id); }, [toggleSelection]);
  ...
  return (
    <div className="media-grid-container">
      <div className="stats-bar">
        ...
        {selectedIds && selectedIds.size > 0 && (<div className="selection-counter"><FaCheck /> <span>Selected: {selectedIds.size}</span></div>)}
      </div>
      <div className="media-grid">
        {mediaList.filter((media) => media != null).map((media, index) => {
          const isLast = index === mediaList.length - 1;
          const isSelected = !!selectedIds && selectedIds.has(media.id);
          return (
            <div key={media.id} className={`media-card ${isSelected ? "is-selected" : ""}`} ref={isLast ? lastElementRef : null} onClick={() => handleCardClick(media)}>
              <div className="card-top-actions">
                {toggleSelection && (<div className={`custom-checkbox ${isSelected ? "checked" : ""}`}>{isSelected && <FaCheck />}</div>)}
                <div className="admin-card-actions">
                  <button className="edit-card-btn" onClick={(e) => handleEditClick(e, media)}><FaEdit /></button>
                  <button className="delete-card-btn" onClick={(e) => handleDeleteClick(e, media)} title="Delete media"><FaTrashAlt /></button>
                </div>
              </div>
